...
class JianshuParser(HTMLParser):
    ...
    def __init__(self):
        super(JianshuParser, self).__init__()
        self.input_dir = '../../../SpiderData/jianshu/Main'
        self.output_dir = '../../../SpiderData/jianshu/Clean'
    ...
    def parse_1html(self, input_file, output_file):
        ...
        f.close()
        # class为_2rhmJa的div区块为正文。把里面标签为p、h1-6、li内容挑出来
        html_text = html_data.select('._2rhmJa')
        if len(html_text) == 1:
    ...
    def parse_1title(self, input_file):
        ...
        f.close()
        header_tag = html_data.select('._1RuRku')
        if len(header_tag) == 1:
