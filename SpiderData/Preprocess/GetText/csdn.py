...
class CSDNParser(HTMLParser):

    def __init__(self):
        super(CSDNParser, self).__init__()
        self.input_dir = '../../../SpiderData/csdn/Main'
        self.output_dir = '../../../SpiderData/csdn/Clean'
    ...
    def parse_1html(self, input_file, output_file):
        ...
        html_text = html_data.select('#content_views')
        if len(html_text) == 1:
    ...
    def parse_1title(self, input_file):
        ...
        f.close()
        header_tag = html_data.select('.article-title-box')
        if len(header_tag) == 1:
            text = header_tag[0].h1.text
            if '\n' in text:
                return str()
            else:
                return text
