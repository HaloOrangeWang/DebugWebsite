...
class OschinaParser(HTMLParser):

    def __init__(self):
        super(OschinaParser, self).__init__()
        self.input_dir = '../../../SpiderData/oschina/Main'
        self.output_dir = '../../../SpiderData/oschina/Clean'
    ...
    def parse_1html(self, input_file, output_file):
        ...
        html_text = html_data.select('.article-detail')
        if len(html_text) == 1:
    ...
    def parse_1title(self, input_file):
        ...
        f.close()
        header_tag = html_data.select('.article-box__title')
        if len(header_tag) == 1:
            text = header_tag[0].a.text
            if '\n' in text:
                return str()
            else:
                return text
